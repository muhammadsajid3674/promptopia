    ...
    return (
        <div className='prompt_card'>
            ...
            {session?.user.id === post.creator._id && pathName === '/profile' && (
                <div className='mt-5 flex-center gap-3 border-t border-gray-250 pt-3'>
                    <p className='font-inter text-sm cursor-pointer green_gradient' onClick={handleEdit}>Edit</p>
                    <p className='font-inter text-sm cursor-pointer orange_gradient' onClick={handleDelete}>delete</p>
                </div>
            )}
        </div>
    )
